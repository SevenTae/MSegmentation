...
from compaCustome2.espnet2.evaluate_train import evalue_iou_miou,evaluateloss
import  time
from  netCompaire.ESPV2.SegmentationModel import EESPNet_Seg,weights_init
import numpy  as np
...
def train_net(net,
    ...
    train_loader = DataLoader(train_d, batch_size=train_batch_size, shuffle=True, num_workers=2, pin_memory=False,drop_last=True)
    val_loader = DataLoader(val_d, shuffle=True, num_workers=2, pin_memory=False, batch_size=val_batch_size,drop_last=True)

    # (Initialize logging)
    experiment = wandb.init(project='compare', resume='allow', anonymous='must')
    experiment.config.update(dict(epochs=epochs, batch_size=train_batch_size, learning_rate=learning_rate,
    ...
    criterion = nn.CrossEntropyLoss(weight=we,ignore_index=ignoreindex)
    start_epoch = 1
    #是否有预训练
    if isPretrain:
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        with tqdm(total=len(train_loader), desc=f'Epoch {epoch}/{epochs}', unit='batch') as pbar:
            for iteration, batch in enumerate(train_loader):  # batchsize=2 一共1487.5个batch
                ...
                out0,out1 = net(images)
                loss = criterion(out0, true_masks)+criterion(out1,true_masks)
        ...
        isevalue = True
        if isevalue == True:
            acc_global, acc, iu, miou = evalue_iou_miou(net, val_loader, device, args.classes,isResize=imgshape_base )
            val_score = miou
            val_loss = evaluateloss(net, val_loader, device, numclass=args.classes, ignoreindex=ignoreindex,isresize=imgshape_base)
...
def get_args():
    ...
    parser.add_argument('--train_batch-size', '-tb', dest='train_batch_size', metavar='TB', type=int, default=4, help='Train_Batch size')
...
if __name__ == '__main__':
    ...
    net = EESPNet_Seg( classes=args.classes,s=2)
